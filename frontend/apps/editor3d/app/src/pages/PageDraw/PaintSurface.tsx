...
export const PaintSurface = ({
  ...
  onSelectionChange
    ...
    const stagePosition = useStageCentering(stageRef, leftPct, leftPanelWidth, leftPanelHeight);

    const NATURAL_WIDTH = rightPanelWidth;  
    const NATURAL_HEIGHT = rightPanelHeight;

    // transform variables
    const transformerRefs = React.useRef<{ [key: string]: Konva.Transformer | null }>({});
    ...
    const clampToLeftPanel = (point: { x: number; y: number }): { x: number; y: number } => {
      return {
    ...
    const isWithinLeftPanel = (point: { x: number; y: number }): boolean => {
      return point.x >= 0 && point.x <= leftPanelWidth && 
             point.y >= 0 && point.y <= leftPanelHeight;
    };

    // Helper function to determine if nodes should be draggable
    const draggableIfToolsNotActive = (activeTool: string, nodeDraggable: boolean): boolean => {
    ...
    const handleStageMouseDown = (e: Konva.KonvaEventObject<MouseEvent | TouchEvent>) => {
      ...
      if ((activeTool === 'draw' || activeTool === 'eraser') && isWithinLeftPanel(stagePoint)) {
    ...
    const handleStageMouseMove = (e: Konva.KonvaEventObject<MouseEvent | TouchEvent>) => {
      ...
      if (isDrawing && currentLineId && (activeTool === 'draw' || activeTool === 'eraser')) {
        const point = stage.getPointerPosition();
        if (!point) return;
        ...
        if (isWithinLeftPanel(stagePoint)) {
          const currentLine = store.lineNodes.find(line => line.id === currentLineId);
          if (currentLine) {
            const updatedPoints = [...currentLine.points, stagePoint.x, stagePoint.y];
      ...
      if (isSelecting && selectionRectRef.current) {
        ...
        setSelectionRect(prev => prev ? ({ // Ensure prev is not null
          ...prev,
          endX: clampedPoint.x,
          endY: clampedPoint.y
        }) : null);
    ...
    useLayoutEffect(() => {
      ...
      if (store.cursorVisible && store.cursorPosition && (activeTool === 'draw' || activeTool === 'eraser')) {
    ...
    }, [store.cursorVisible, store.cursorPosition, activeTool, brushColor, brushSize]);
    ...
    const renderNode = (node: Node | LineNode) => {
        ...
        const handleNodeDragStart = (e: Konva.KonvaEventObject<DragEvent>, nodeId:string) => {
          const targetNode = e.target as Konva.Node & { lastX?: number; lastY?: number };
        ...
        const handleNodeDragMove = (e: Konva.KonvaEventObject<DragEvent>, nodeId: string) => {
          const targetNode = e.target as Konva.Node & { lastX?: number; lastY?: number };
      ...
      const renderTransformer = () => {
        if (isSelected) {
          return (
            <Transformer
              ref={(ref: Konva.Transformer | null) => { // Added type for ref
                if (ref) {
              ...
              padding={5}
              ignoreStroke={true}
              onTransformEnd={(e: Konva.KonvaEventObject<Event>) => { // Typed event
                ...
                if (isLineNode) {
                  ...
                  store.updateNode(nodeId, {
                    ...
                    offsetX: finalOffsetX,
                    offsetY: finalOffsetY
                  }, true); // Assuming shouldSaveState is true for regular nodes
                }
              }}
            />
          );
      ...
      if (node.type === 'line') {
        ...
        const handleLineDragStart = (e: Konva.KonvaEventObject<DragEvent>, nodeId:string) => {
          const targetNode = e.target as Konva.Node & { lastX?: number; lastY?: number };
          targetNode.lastX = targetNode.x();
          targetNode.lastY = targetNode.y();
          
        }
        const handleLineDragMove = (e: Konva.KonvaEventObject<DragEvent>, nodeId: string) => {
          const targetNode = e.target as Konva.Node & { lastX?: number; lastY?: number };
        ...
        const handleLineDragEnd = (e: Konva.KonvaEventObject<DragEvent>, nodeId: string) => {
        ...
        return (
          <React.Fragment key={lineNode.id}>
            <Line
              ...
              strokeWidth={isSelected ? (lineNode.strokeWidth || 0) + 2 : lineNode.strokeWidth} // Ensure strokeWidth is number
              tension={0.5}
              lineCap="round"
              lineJoin="round"
              ...
              draggable={draggableIfToolsNotActive(activeTool, lineNode.draggable)}
    ...
    return (<SplitPane
      ...
      right={
        <div ref={rightContainerRef} className="flex h-full w-full items-center justify-center overflow-hidden p-4">
          <Stage
