...
export class MouseControls {
  ...
  onMouseClick() {
    ...
    this.sceneManager?.scene.selectableBones.forEach((child: THREE.Object3D) => {
      if (child.name != "") {
        if (
          child.type == "Mesh" ||
          child.type == "Object3D" ||
    ...
    if(intersects.length <= 0){
      ...
      intersects = this.raycaster.intersectObjects(interactable, true);
    }
    ...
    if (intersects.length > 0) {
      if (intersects[0].object.type != "GridHelper") {
        ...
        while (currentObject.parent && currentObject.parent.type !== "Scene" && currentObject.userData.media_id !== "Bone") {
