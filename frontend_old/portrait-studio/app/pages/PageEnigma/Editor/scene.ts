...
class Scene {
  ...
  targetBones: THREE.Object3D<THREE.Object3DEventMap>[] = [];
  selectableBones: THREE.Object3D<THREE.Object3DEventMap>[] = [];
  ...
  async instantiate(
    ...
    if (name == "Box") {
      ...
      geometry = new THREE.SphereGeometry(2, 18, 12);
  ...
  async loadGlbWithPlaceholder(
    ...
    // Traverse the scene to find SkinnedMeshes
    child_result.traverse(async (object) => {
      if (object instanceof THREE.SkinnedMesh && !done) {
        // Check if the SkinnedMesh has a skeleton
        if (object.skeleton) {
          const OOI = {};

          done = true;

          object.skeleton.bones.forEach((n) => {
            if (n.name === "mixamorigRightShoulder" || n.name === "shoulderr") OOI.RightShoulder = n;
            if (n.name === "mixamorigRightArm" || n.name === "shoulderr") OOI.RightArm = n;
            if (n.name === "mixamorigRightForeArm"|| n.name === "forearm_stretchr") OOI.RightForeArm = n;
            if (n.name === "mixamorigRightHand" || n.name === "handr") OOI.RightHand = n;
            if (n.name === "mixamorigLeftShoulder" || n.name === "shoulderl") OOI.LeftShoulder = n;
            if (n.name === "mixamorigLeftArm" || n.name === "shoulderl") OOI.LeftArm = n;
            if (n.name === "mixamorigLeftForeArm" || n.name === "forearm_stretchl") OOI.LeftForeArm = n;
            if (n.name === "mixamorigLeftHand" || n.name === "handl") OOI.LeftHand = n;
            if (n.name === "mixamorigRightUpLeg" || n.name === "thigh_stretchr") OOI.RightUpLeg = n;
            if (n.name === "mixamorigRightFoot" || n.name === "footr") OOI.RightFoot = n;
            if (n.name === "mixamorigRightLeg" || n.name === "leg_stretchr") OOI.RightLeg = n;
            if (n.name === "mixamorigLeftUpLeg" || n.name === "thigh_stretchl") OOI.LeftUpLeg = n;
            if (n.name === "mixamorigLeftFoot" || n.name === "footl") OOI.LeftFoot = n;
            if (n.name === "mixamorigLeftLeg" || n.name === "leg_stretchl") OOI.LeftLeg = n;
            if (n.name === "mixamorigNeck" || n.name === "neckx") OOI.Neck = n;
            if (n.name === "neckx") OOI.IsMixamo = false;
            if (n.name === "mixamorigNeck") OOI.IsMixamo = true;

          });

          child_result.userData["bones"] = OOI;

          const Controls = {};

          if (OOI.Neck) {
            const Neck = (await this.instantiate("Bone")) as THREE.Object3D;

            const RightLeg = (await this.instantiate("Bone")) as THREE.Object3D;
            const RightFoot = (await this.instantiate(
              "Bone",
            )) as THREE.Object3D;
            const RightUpLeg = (await this.instantiate(
              "Bone",
            )) as THREE.Object3D;

            RightUpLeg.rotateX(Math.PI);
            RightUpLeg.rotateY(-Math.PI);
            RightFoot.rotateX((Math.PI / 180) * 45);

            const LeftLeg = (await this.instantiate("Bone")) as THREE.Object3D;
            const LeftFoot = (await this.instantiate("Bone")) as THREE.Object3D;
            const LeftUpLeg = (await this.instantiate(
              "Bone",
            )) as THREE.Object3D;

            LeftUpLeg.rotateX(Math.PI);
            LeftUpLeg.rotateY(-Math.PI);
            LeftFoot.rotateX((Math.PI / 180) * 45);

            const RightArm = (await this.instantiate("Bone")) as THREE.Object3D;
            const RightHand = (await this.instantiate(
              "Bone",
            )) as THREE.Object3D;
            const RightForeArm = (await this.instantiate(
              "Bone",
            )) as THREE.Object3D;

            const LeftArm = (await this.instantiate("Bone")) as THREE.Object3D;
            const LeftHand = (await this.instantiate("Bone")) as THREE.Object3D;
            const LeftForeArm = (await this.instantiate(
              "Bone",
            )) as THREE.Object3D;

            Controls["RightArm"] = RightArm;
            Controls["RightForeArm"] = RightForeArm;
            Controls["RightHand"] = RightHand;
            Controls["LeftArm"] = LeftArm;
            Controls["LeftForeArm"] = LeftForeArm;
            Controls["LeftHand"] = LeftHand;

            Controls["LeftLeg"] = LeftLeg;
            Controls["LeftFoot"] = LeftFoot;
            Controls["LeftUpLeg"] = LeftUpLeg;
            Controls["RightLeg"] = RightLeg;
            Controls["RightFoot"] = RightFoot;
            Controls["RightUpLeg"] = RightUpLeg;

            Controls["Neck"] = Neck;

            child_result.userData["Controls"] = Controls;
            ...
            [
              ...
              LeftUpLeg,
              Neck,
            ].forEach((bone: THREE.Object3D) => {
              bone.userData.media_id = "Bone";
              bone.renderOrder = 999;
              bone.material.depthTest = false;
              bone.material.depthWrite = false;
              bone.layers.set(1);
            });

            // Linking the control objects to the OOI bones
            OOI.RightArm.add(RightArm);
            OOI.RightForeArm.add(RightForeArm);
            OOI.RightHand.add(RightHand);

            OOI.LeftArm.add(LeftArm);
            OOI.LeftHand.add(LeftHand);
            OOI.LeftForeArm.add(LeftForeArm);

            OOI.RightFoot.add(RightFoot);
            OOI.RightLeg.add(RightLeg);
            OOI.RightUpLeg.add(RightUpLeg);

            OOI.LeftFoot.add(LeftFoot);
            OOI.LeftLeg.add(LeftLeg);
            OOI.LeftUpLeg.add(LeftUpLeg);

            OOI.Neck.add(Neck);

            this.selectableBones.push(
              RightArm,
              RightForeArm,
              RightHand,
              LeftArm,
            ...
            // Add to targetBones for whatever processing or updates needed
            this.targetBones.push(
              { target: RightArm, bone: OOI.RightArm },
              { target: RightForeArm, bone: OOI.RightForeArm },
              { target: RightHand, bone: OOI.RightHand },
              { target: LeftArm, bone: OOI.LeftArm },
              { target: LeftForeArm, bone: OOI.LeftForeArm },
              { target: LeftHand, bone: OOI.LeftHand },
              { target: RightFoot, bone: OOI.RightFoot },
              { target: RightLeg, bone: OOI.RightLeg },
              { target: RightUpLeg, bone: OOI.RightUpLeg },
              { target: LeftFoot, bone: OOI.LeftFoot },
              { target: LeftLeg, bone: OOI.LeftLeg },
              { target: LeftUpLeg, bone: OOI.LeftUpLeg },
              { target: Neck, bone: OOI.Neck },
            );
          }
        }
      }
    });
    await this.delay(150); // TODO: Fix this and make it wait for the loop above ^^
    this.updateSurfaceIdAttributeToMesh(this.scene);
    return child_result;
  }
  ...
  updateIKs() {
    ...
    this.targetBones.forEach((targetBone) => {
      const target = targetBone.target;
      const bone = targetBone.bone;

      if (bone) {
        bone.rotation.copy(target.rotation);
        // bone.position.copy(new THREE.Vector3(0,0,0));
      }
    });
  }
