...
import { globalIPAMediaToken, queueData, updateQueueItem } from "../signals";
...
export class VideoGeneration {
  ...
  async generateFrame() {
    if (!this.editor.generating_preview && this.editor.rawRenderer) {
      this.editor.render_width = this.editor.getRenderDimensions().width;
      this.editor.render_height = this.editor.getRenderDimensions().height;
      ...
      const currentIndex = queueData.value.length - 1;

      updateQueueItem(currentIndex, imgData, true);
      ...
      try {
        ...
        updateQueueItem(currentIndex, url, false); // Update the queue item with the new image URL
