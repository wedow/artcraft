...
pub fn lcm_pipeline(args: Args<'_>) -> Result<RgbImage> {
  ...
  let Args { prompt, uncond_prompt, cfg_scale, i2i_strength, configs, model_cache, prompt_cache, app, image, use_flash_attn } = args;
  ...
  let vae = match maybe_vae {
    ...
    None => {
      ...
      if true {
        ...
        app.emit("notification", NotificationEvent::ModelDownloadStarted { model_name: repo, model_type: ModelType::Vae })?;
      }
      ...
      if notify_download_complete {
        app.emit("notification", NotificationEvent::ModelDownloadComplete { model_name: repo, model_type: ModelType::Vae })?;
      }
  ...
  let unet = match maybe_unet {
    ...
    None => {
      ...
      if notify_download_complete {
        app.emit("notification", NotificationEvent::ModelDownloadComplete { model_name: "sdxl-turbo-unet", model_type: ModelType::Unet })?;
      }
