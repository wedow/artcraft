...
use log::info;
use tauri::AppHandle;
use crate::core::commands::enqueue::image_edit::nano_banana::handle_nano_banana_edit_artcraft::handle_nano_banana_edit_artcraft;

pub async fn handle_nano_banana_edit(
  ...
  for provider in priority.iter() {
    match provider {
      ...
      Provider::Artcraft => {
        info!("Dispatching Nano Banana (edit) via Artcraft...");
        return handle_nano_banana_edit_artcraft(
          request, 
          app,
          app_data_root,
          app_env_configs, 
