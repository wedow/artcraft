...
const WEBSOCKET_WSS_URL: &str = "wss://grok.com/ws/imagine/listen";
const WEBSOCKET_HTTPS_URL: &str = "https://grok.com/ws/imagine/listen";

pub async fn create_listen_websocket_raw_non_websocket() -> Result<(), GrokError> {
  ...
  let builder = client
      .get(WEBSOCKET_HTTPS_URL)
      .header("Host", "grok.com")
      .header("Accept", "*/*")
      .header("Cookie", cookies)
      .header("Connection", "Upgrade")
      .header("Sec-WebSocket-Version", "13")
      .header("Sec-WebSocket-Key", "JpMsHjXLPEmOsa89FQBeHA==")
...
mod tests {
  ...
  async fn create() -> AnyhowResult<()> {
    ...
    let _result = create_listen_websocket_raw_non_websocket().await;
    log::logger().flush();
    assert_eq!(1, 2);
    Ok(())
  }
