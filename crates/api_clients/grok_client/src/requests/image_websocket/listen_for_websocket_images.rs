...
use crate::requests::image_websocket::messages::image_data_message::MessageImageData;
use log::info;
...
pub async fn listen_for_website_images(args: ListenForWebsocketImagesArgs<'_>) -> Result<ImageResults, GrokError> {
...
mod tests {
  ...
  async fn prompt() -> AnyhowResult<()> {
    ...
    let images = listen_for_website_images(ListenForWebsocketImagesArgs {
      websocket: &websocket,
