...
pub async fn enqueue_veo_3p1_first_last_frame_image_to_video_webhook<R: IntoUrl>(
  ...
  let duration = args.duration
      .map(|resolution| match resolution {
        EnqueueVeo3p1FirstLastFrameImageToVideoDurationSeconds::Four => "4",
        EnqueueVeo3p1FirstLastFrameImageToVideoDurationSeconds::Six => "6",
        EnqueueVeo3p1FirstLastFrameImageToVideoDurationSeconds::Eight=> "8",
      })
      .map(|s| s.to_string());
