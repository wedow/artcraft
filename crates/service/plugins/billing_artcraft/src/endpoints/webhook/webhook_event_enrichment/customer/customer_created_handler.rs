...
pub fn customer_created_handler(customer: Customer) -> Result<EnrichedWebhookEvent, StripeArtcraftWebhookError> {
  ...
  if let Some(metadata) = &customer.metadata {
    ...
    if let Some(user_token) = &maybe_user_token {
      ...
      maybe_action = Some(ArtcraftBillingAction::CustomerCreated(link));
    }
  }
