...
#[utoipa::path(
  ...
  path = "/v1/generate/video/multi_function/sora_2_pro",
  responses(
    (status = 200, description = "Success", body = Sora2ProMultiFunctionVideoGenResponse),
  ),
  params(
    ("request" = Sora2ProMultiFunctionVideoGenRequest, description = "Payload for Request"),
  )
)]
pub async fn sora_2_pro_multi_function_video_gen_handler(
  http_request: HttpRequest,
  request: Json<Sora2ProMultiFunctionVideoGenRequest>,
  server_state: web::Data<Arc<ServerState>>
) -> Result<Json<Sora2ProMultiFunctionVideoGenResponse>, CommonWebError> {
  ...
  if let Some(image_url) = maybe_image_url {
    ...
    let duration = match request.duration {
      ...
      Some(Sora2ProMultiFunctionVideoGenDuration::EightSeconds) => EnqueueSora2ProImageToVideoDurationSeconds::Eight,
      Some(Sora2ProMultiFunctionVideoGenDuration::TwelveSeconds) => EnqueueSora2ProImageToVideoDurationSeconds::Twelve,
      None => EnqueueSora2ProImageToVideoDurationSeconds::Eight,
    };

    let aspect_ratio = match request.aspect_ratio {
      Some(Sora2ProMultiFunctionVideoGenAspectRatio::Auto) => EnqueueSora2ProImageToVideoAspectRatio::Auto,
      Some(Sora2ProMultiFunctionVideoGenAspectRatio::SixteenByNine) => EnqueueSora2ProImageToVideoAspectRatio::SixteenByNine,
      Some(Sora2ProMultiFunctionVideoGenAspectRatio::NineBySixteen) => EnqueueSora2ProImageToVideoAspectRatio::NineBySixteen,
      None => EnqueueSora2ProImageToVideoAspectRatio::SixteenByNine,
    };

    let resolution = match request.resolution {
      Some(Sora2ProMultiFunctionVideoGenResolution::Auto) => EnqueueSora2ProImageToVideoResolution::Auto,
      Some(Sora2ProMultiFunctionVideoGenResolution::SevenTwentyP) => EnqueueSora2ProImageToVideoResolution::SevenTwentyP,
      Some(Sora2ProMultiFunctionVideoGenResolution::TenEightyP) => EnqueueSora2ProImageToVideoResolution::TenEightyP,
      None => EnqueueSora2ProImageToVideoResolution::TenEightyP,
    };
    ...
    fal_result = enqueue_sora_2_pro_image_to_video_webhook(args)
        .await
        .map_err(|err| {
          warn!("Error calling enqueue_sora_2_pro_image_to_video_webhook: {:?}", err);
          CommonWebError::ServerError
        })?;
    ...
    let duration = match request.duration {
      ...
      Some(Sora2ProMultiFunctionVideoGenDuration::EightSeconds) => EnqueueSora2ProTextToVideoDurationSeconds::Eight,
      Some(Sora2ProMultiFunctionVideoGenDuration::TwelveSeconds) => EnqueueSora2ProTextToVideoDurationSeconds::Twelve,
      None => EnqueueSora2ProTextToVideoDurationSeconds::Eight,
    };
    ...
    fal_result = enqueue_sora_2_pro_text_to_video_webhook(args)
        .await
        .map_err(|err| {
          warn!("Error calling enqueue_sora_2_pro_text_to_video_webhook: {:?}", err);
          CommonWebError::ServerError
        })?;
  }
  ...
  Ok(Json(Sora2ProMultiFunctionVideoGenResponse {
    success: true,
