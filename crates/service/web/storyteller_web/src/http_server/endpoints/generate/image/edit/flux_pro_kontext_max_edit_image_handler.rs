...
use artcraft_api_defs::generate::image::edit::flux_pro_kontext_max_edit_image::{FluxProKontextMaxEditImageNumImages, FluxProKontextMaxEditImageRequest, FluxProKontextMaxEditImageResponse};
...
#[utoipa::path(
  ...
  path = "/v1/generate/image/edit/flux_pro_kontext_max",
  responses(
    (status = 200, description = "Success", body = FluxProKontextMaxEditImageResponse),
  ),
  params(
...
pub async fn flux_pro_kontext_max_edit_image_handler(
  ...
  let num_images = match request.num_images {
    ...
    Some(FluxProKontextMaxEditImageNumImages::Two) => FluxProKontextMaxNumImages::Two,
    Some(FluxProKontextMaxEditImageNumImages::Three) => FluxProKontextMaxNumImages::Three,
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_creator_user_token: maybe_user_session
        .as_ref()
        .map(|s| &s.user_token),
    maybe_model_type: Some(ModelType::FluxProKontextMax),
