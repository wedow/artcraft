...
use fal_client::requests::webhook::video::enqueue_veo_3_image_to_video_webhook::{enqueue_veo_3_image_to_video_webhook, Veo3Args, Veo3AspectRatio, Veo3Duration, Veo3Resolution};
...
#[utoipa::path(
  post,
  tag = "Generate Videos",
  path = "/v1/generate/video/veo_3_image_to_video",
  responses(
    (status = 200, description = "Success", body = GenerateVeo3ImageToVideoResponse),
  ),
  params(
    ("request" = GenerateVeo3ImageToVideoRequest, description = "Payload for Request"),
  )
)]
pub async fn generate_veo_3_image_to_video_handler(
  http_request: HttpRequest,
  request: Json<GenerateVeo3ImageToVideoRequest>,
  server_state: web::Data<Arc<ServerState>>
) -> Result<Json<GenerateVeo3ImageToVideoResponse>, CommonWebError> {
  ...
  let fal_result = enqueue_veo_3_image_to_video_webhook(args)
      .await
      .map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_creator_user_token: maybe_user_session
        .as_ref()
        .map(|s| &s.user_token),
    maybe_model_type: Some(ModelType::Veo3),
