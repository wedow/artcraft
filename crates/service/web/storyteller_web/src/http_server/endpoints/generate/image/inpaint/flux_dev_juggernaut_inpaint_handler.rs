...
use artcraft_api_defs::generate::image::inpaint::flux_dev_juggernaut_inpaint_image::{FluxDevJuggernautInpaintImageNumImages, FluxDevJuggernautInpaintImageRequest, FluxDevJuggernautInpaintImageResponse};
...
/// Flux Dev juggernaut inpainting
#[utoipa::path(
  post,
  tag = "Generate Images (Inpaint)",
  path = "/v1/generate/image/inpaint/flux_dev_juggernaut",
  responses(
    (status = 200, description = "Success", body = FluxDevJuggernautInpaintImageResponse),
  ),
  params(
    ("request" = FluxDevJuggernautInpaintImageRequest, description = "Payload for Request"),
  )
)]
pub async fn flux_dev_juggernaut_inpaint_image_handler(
  http_request: HttpRequest,
  request: Json<FluxDevJuggernautInpaintImageRequest>,
  server_state: web::Data<Arc<ServerState>>
) -> Result<Json<FluxDevJuggernautInpaintImageResponse>, CommonWebError> {
  ...
  let num_images = match request.num_images {
    ...
    Some(FluxDevJuggernautInpaintImageNumImages::Two) => FluxDevJuggernautInfillNumImages::Two,
    Some(FluxDevJuggernautInpaintImageNumImages::Three) => FluxDevJuggernautInfillNumImages::Three,
    Some(FluxDevJuggernautInpaintImageNumImages::Four) => FluxDevJuggernautInfillNumImages::Four,
    None => FluxDevJuggernautInfillNumImages::One, // Default to One
  };
  ...
  let fal_result = enqueue_flux_dev_juggernaut_infill_webhook(args)
      .await
      .map_err(|err| {
        warn!("Error calling enqueue_flux_dev_juggernaut_infill_webhook: {:?}", err);
        CommonWebError::ServerError
      })?;
  ...
  Ok(Json(FluxDevJuggernautInpaintImageResponse {
    success: true,
