...
impl BasicJwtClaims {
  pub fn parse(raw_jwt: &str) -> Result<Self, JwtError> {
    let [_header_str, claims_str, _signature_str] = split_components(raw_jwt)?;

    let raw_json = decode_base64(claims_str)?;

    let default_claims : BasicJwtClaimsRaw = serde_json::from_str(&raw_json)?;
...
fn decode_base64(raw: &str) -> Result<String, JwtError> {
...
mod tests {
  ...
  mod runwayml {
    ...
    fn test_parse() {
      ...
      let claims = JwtClaims::parse(jwt).unwrap();
