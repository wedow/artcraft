...
pub async fn create_password_reset(pool: &MySqlPool, user: &UserRecordForLogin, ip_address: String, secret_key: String) -> AnyhowResult<()> {
    ...
    // Definitely remove the empty string default as well!

    sqlx::query!(
        r#"
INSERT INTO password_resets
(token, user_token, secret_key, current_password_version, ip_address_creation, ip_address_redemption, expires_at)
